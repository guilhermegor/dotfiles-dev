...
## Lesson mirrors (`lessons/`) — not a feature directory

`.specs/lessons/<store>-lessons.md` (e.g. `.specs/lessons/blueprintx-lessons.md`) is this
...
wrote to be read; `.specs/lessons/` holds output a script wrote to be `grep`ped.
...
- Shipped or reference documentation → `docs/` (see the `lessons/` exception above
...
- Anything meant to outlive the feature it was written for (ADRs, README,
  CLAUDE.md changes) — the `lessons/` mirrors above are the one exception: they
