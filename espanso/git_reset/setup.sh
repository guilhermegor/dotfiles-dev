#!/usr/bin/env bash
# setup.sh - Configure PATH and create terminal wrapper for :gitreset

set -e

BIN_DIR="$HOME/bin"
WRAPPER_NAME=":gitreset"

echo "Setting up :gitreset terminal command..."
...
#!/usr/bin/env bash
exec bash -lc 'git reset --hard HEAD && git clean -fd && code . --reuse-window'
WRAPPER
...
echo "  :gitreset"
echo ""
echo "WARNING: This command will discard uncommitted changes and remove untracked files."
