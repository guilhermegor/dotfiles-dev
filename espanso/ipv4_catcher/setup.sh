#!/usr/bin/env bash
# setup.sh - Configure PATH and create terminal wrapper for :ipv4_catcher

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BIN_DIR="$HOME/bin"
WRAPPER_NAME=":ipv4_catcher"

echo "Setting up :ipv4_catcher terminal command..."
...
#!/usr/bin/env bash
exec "$HOME/.config/espanso/packages/ipv4_catcher/ipv4_catcher.sh" "$@"
WRAPPER
