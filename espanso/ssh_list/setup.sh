#!/usr/bin/env bash
# setup.sh - Configure PATH and create terminal wrapper for :sshlist

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BIN_DIR="$HOME/bin"
WRAPPER_NAME=":sshlist"

echo "Setting up :sshlist terminal command..."
...
#!/usr/bin/env bash
exec "$HOME/.config/espanso/packages/ssh_list/ssh_list.sh" "$@"
WRAPPER
