...
| `git_sync_origin` | `:git_sync_origin` | Sync all local tracking branches to match origin (fast-forward only; deletes branches whose work is already on origin, including squash/rebase merges; warns and skips branches with genuinely unmerged commits) |
