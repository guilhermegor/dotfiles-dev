...
| what | size |
|---|---|
| `~/Insync/guirodrigues.gor@gmail.com/` (one Google account) | **1.3 T** |
...
⚠️ Deleting `~/Insync/` while Insync is running propagates the deletion to Google Drive. The
...
3. **Uninstall the package.** `sudo apt remove --purge insync`, verified against
   `dpkg -l | grep insync` being empty. Touches only the local machine — Google Drive keeps
...
- **OneDriver** — OneDrive-only; not applicable (the account here is Google Drive).
